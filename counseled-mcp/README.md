...
| *(no access tool)* | Family membership is managed **by hand** in the tracker page (Edit → Family access, max 6). Deliberate: AI assistants refuse to modify access controls, so there is no tool to ask them to use — the page's guardrails live in firestore.rules. |
| `create_page` / `update_page` / `list_pages` / `get_page` | Freeform Claude-built pages ("artifacts") in the family's tracker. Served sandboxed from `/p/{id}`: CSP blocks all external network/frames/forms (`connect-src 'none'` etc.), viewed inside `<iframe sandbox="allow-scripts">` (opaque origin — no cookies/storage/Firebase), live tracker state injected read-only via a nonce-verified MessageChannel (`Counseled.ready`/`onUpdate`). Creation lint rejects iframes/objects/meta-refresh; 400KB max, 20/tracker, journal provenance. |
